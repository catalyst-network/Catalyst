...
namespace Ipfs.Core.CoreApi
{
    internal class BootstrapApi : IBootstrapApi
    {
        ...
        public BootstrapApi(IpfsEngine ipfs) { this._ipfs = ipfs; }
        ...
        {
            foreach (var a in Defaults) await AddAsync(a, cancel).ConfigureAwait(false);
        ...
        {
            if (_ipfs.Options.Discovery.BootstrapPeers != null) return _ipfs.Options.Discovery.BootstrapPeers;
        ...
        {
            await _ipfs.Config.SetAsync("Bootstrap", JToken.FromObject(new string[0]), cancel).ConfigureAwait(false);
        }
