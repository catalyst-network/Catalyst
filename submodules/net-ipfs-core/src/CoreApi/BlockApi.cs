...
namespace Ipfs.Core.CoreApi
{
    ...
    internal class BlockApi : IBlockApi
    {
        ...
        public BlockApi(IpfsEngine ipfs) { this._ipfs = ipfs; }
        ...
        public async Task<IDataBlock> GetAsync(Cid id, CancellationToken cancel = default)
        {
            ...
            {
                var bitswapGet = _ipfs.Bitswap.GetAsync(id, queryCancel.Token).ConfigureAwait(false);
            ...
            CancellationToken cancel = default)
        {
            ...
            if (encoding != "base58btc") cid.Encoding = encoding;
            var block = new DataBlock
            {
                DataBytes = data,
                Id = cid,
            ...
            else
            {
                ...
                if (_ipfs.IsStarted) await _ipfs.Dht.ProvideAsync(cid, false, cancel).ConfigureAwait(false);
        ...
        {
            if (id.Hash.IsIdentityHash) return id;
        ...
        {
            if (id.Hash.IsIdentityHash) return await GetAsync(id, cancel).ConfigureAwait(false);
