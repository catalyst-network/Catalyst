...
namespace Ipfs.Core.BlockExchange
{
    ...
    public class Bitswap : IService
    {
        ...
        private readonly ConcurrentDictionary<Peer, BitswapLedger> _peerLedgers =
            new ConcurrentDictionary<Peer, BitswapLedger>();
        ...
        /// </summary>
        /// <seealso cref="Ipfs.Abstractions.CoreApi.IStatsApi" />
        ...
        public Task StartAsync()
        {
            ...
            foreach (var protocol in Protocols) Swarm.AddProtocol(protocol);
        ...
        public Task StopAsync()
        {
            ...
            Swarm.ConnectionEstablished -= Swarm_ConnectionEstablished;
            foreach (var protocol in Protocols) Swarm.RemoveProtocol(protocol);

            foreach (var cid in _wants.Keys) Unwant(cid);
        ...
        /// </returns>
        /// <seealso cref="Ipfs.Abstractions.CoreApi.IBitswapApi.LedgerAsync(IPeer, CancellationToken)" />
        public BitswapLedger PeerLedger(Peer peer)
        {
            if (_peerLedgers.TryGetValue(peer, out var ledger)) return ledger;
...
#pragma warning disable VSTHRD100 // Avoid async void methods
        private async void Swarm_ConnectionEstablished(object sender, IPeerConnection connection)
#pragma warning restore VSTHRD100 // Avoid async void methods
        ...
        {
            if (Log.IsDebugEnabled) Log.Debug($"{peer} wants {id}");
        ...
        {
            if (Log.IsDebugEnabled) Log.Debug($"Unwant {id}");
        ...
        public int Found(IDataBlock block)
        {
            ...
            {
                foreach (var consumer in want.Consumers) consumer.SetResult(block);
        ...
        private async Task SendWantListToAllAsync(IEnumerable<WantedBlock> wants, bool full)
        {
            ...
            try
            {
                var tasks = Enumerable.ToArray<Task>(Swarm.KnownPeers
        ...
        private async Task SendWantListAsync(Peer peer, IEnumerable<WantedBlock> wants, bool full)
        {
            ...
            foreach (var protocol in Protocols)
                try
                {
                    ...
                    {
                        await protocol.SendWantsAsync(stream, wants, full).ConfigureAwait(false);
                    }
