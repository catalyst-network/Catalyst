...
namespace ADL.KeySigner
{
    public class TransactionSigner
    {
        ...
        {
            transaction.Sign(new AtlasECKey(privateKey, true));
...
#if !DOTNET35
        private async Task<string> SignTransactionAsync(IAtlasExternalSigner externalSigner, Transaction transaction)
        {
            await transaction.SignExternallyAsync(externalSigner).ConfigureAwait(false);
            return transaction.GetRLPEncoded().ToHex();
        }

        private async Task<string> SignTransactionAsync(IAtlasExternalSigner externalSigner, TransactionChainId transaction)
        {
            await transaction.SignExternallyAsync(externalSigner).ConfigureAwait(false);
            return transaction.GetRLPEncoded().ToHex();
        }
        ...
        public Task<string> SignTransactionAsync(IAtlasExternalSigner externalSigner, string to, BigInteger amount, BigInteger nonce)
        {
            var transaction = new Transaction(to, amount, nonce);
            return SignTransactionAsync(externalSigner, transaction);
        }

        public Task<string> SignTransactionAsync(IAtlasExternalSigner externalSigner, string to, BigInteger amount, BigInteger nonce, string data)
        {
            var transaction = new Transaction(to, amount, nonce, data);
            return SignTransactionAsync(externalSigner, transaction);
        }
       ...
       public Task<string> SignTransactionAsync(IAtlasExternalSigner externalSigner, Chain chain, string to, BigInteger amount,
            BigInteger nonce)
        {
            return SignTransactionAsync(externalSigner, (int)chain, to, amount, nonce);
        }

        public Task<string> SignTransactionAsync(IAtlasExternalSigner externalSigner, BigInteger chainId, string to, BigInteger amount,
            BigInteger nonce)
        {
            var transaction = new TransactionChainId(to, amount, nonce, chainId);
            return SignTransactionAsync(externalSigner, transaction);
