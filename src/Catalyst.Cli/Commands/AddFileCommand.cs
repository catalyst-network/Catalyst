...
namespace Catalyst.Cli.Commands
{
    public sealed class AddFileCommand : BaseMessageCommand<AddFileToDfsRequest, AddFileToDfsResponse, AddFileOptions>
    {
        ...
        protected override bool ExecuteCommand(AddFileOptions options) { return File.Exists(options.File); }
        ...
        protected override void ResponseMessage(AddFileToDfsResponse response)
        {
            ...
            if (responseCode == FileTransferResponseCodeTypes.Failed ||
                responseCode == FileTransferResponseCodeTypes.Finished)
                CommandContext.UserOutput.WriteLine("File transfer completed, Response: " + responseCode.Name +
                    " Dfs Hash: " + response.DfsHash);
        }
    }
}
