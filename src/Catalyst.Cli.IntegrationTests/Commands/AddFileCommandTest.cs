        ...
        public async Task Cli_Can_Send_Add_File_Request(string fileName, bool expectedResult)
        {
            using (var container = ContainerBuilder.Build())
            {
                ...
                using (container.BeginLifetimeScope(CurrentTestName))
                {
                    ...
                    if (expectedResult)
                    {
                        NodeRpcClient.Received(1).SendMessage(Arg.Is<IOutboundDto>(x => x.Message.Descriptor != null && x.Message.Descriptor.Name.Equals(AddFileToDfsRequest.Descriptor.Name)));
                    }
                }   
            }
        }
