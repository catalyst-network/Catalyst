...
namespace Catalyst.Helpers.Redis.UnitTests
{
    ...
    public class UT_RedisConnector
    {
        ...
        {
            for (var k = start; k < end; k++) Assert.IsTrue(Db.StringSet($"mykey:{k}", k));
        }

        private static void Reader(int start, int end)
        {
            for (var k = start; k < end; k++) Assert.AreEqual(k, Db.StringGet($"mykey:{k}"));
        }
        ...
        public void KeyDoesNotExists()
        {
            ...
            if (ret.HasValue) Assert.Fail("Expected no value with this key in Redis");
        }
        ...
        public void WriteReadMany()
        {
            ...
            for (var i = 0; i < num; i++) Assert.AreEqual(i, Db.StringGet($"mykey:{i}"));
        }
