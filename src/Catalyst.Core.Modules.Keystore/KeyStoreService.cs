...
namespace Catalyst.Core.Modules.Keystore
{
    ...
    public class KeyStoreService : IKeyStoreService
    {
        ...
        static ILog log = LogManager.GetLogger(typeof(KeyStoreService));
        ...
        /// </param>
        public KeyStoreService(string folder) { _folder = folder; }
        ...
        FileStore<string, EncryptedKey> Store
        {
            get
            {
                ...
                {
                    Folder = folder,
                    NameToKey = (name) => Encoding.UTF8.GetBytes((string) name).ToBase32(),
                    KeyToName = (key) => Encoding.UTF8.GetString((byte[]) Base32.Decode(key))
                };
        ...
        public async Task<string> GetPublicKeyAsync(string name, CancellationToken cancel = default)
        {
            ...
            if (ekey != null)
            {
                UseEncryptedKey(ekey, key =>
                {
                    ...
                    // Add protobuf cruft.
                    var publicKey = new Lib.Cryptography.Proto.PublicKey
                    {
                        Data = spki
                    };
                    switch (kp.Public) 
