...
namespace Catalyst.Common.UnitTests.P2P
{
    public static class NetworkTests
    {
        static NetworkTests()
        {
            NetworksAndExpectations = new List<Protocol.Common.Network>
            {
                Protocol.Common.Network.Devnet, 
                Protocol.Common.Network.Mainnet, 
                Protocol.Common.Network.Testnet
            }.Select(n => new[] {n.ToString(), n as object}).ToList();
        }

        private static List<object[]> NetworksAndExpectations { get; }
