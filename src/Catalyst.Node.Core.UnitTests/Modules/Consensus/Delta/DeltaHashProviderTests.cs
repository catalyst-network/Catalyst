...
namespace Catalyst.Node.Core.UnitTests.Modules.Consensus.Delta
{
    public sealed class DeltaHashProviderTests : SelfAwareTestBase
    {
        ...
        {
            return DateTime.FromOADate(Offset + i);
        }
        ...
        private void BuildDeltasAndSetCacheExpectations(int deltaCount)
        {
            var deltas = Enumerable.Range(0, deltaCount)
               .Select(i =>
                {
                    var delta = DeltaHelper.GetDelta(previousDeltaHash:
                        GetHash(i - 1).ToBytes(),
                        timestamp: (uint) i + Offset);
                    return delta;
                })
               .ToList();
        ...
        private void ExpectTryGetDelta(string hash, Protocol.Delta.Delta delta)
        {
            _deltaCache.TryGetDelta(hash, out Arg.Any<Protocol.Delta.Delta>())
               .Returns(ci =>
                {
                    ci[1] = delta;
                    return true;
