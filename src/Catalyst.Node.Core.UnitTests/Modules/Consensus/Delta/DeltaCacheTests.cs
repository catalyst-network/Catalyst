...
namespace Catalyst.Node.Core.UnitTests.Modules.Consensus.Delta
{
    public class DeltaCacheTests
    {
        ...
        public void TryGetDelta_Should_Not_Hit_The_Dfs_Or_Store_Delta_When_Delta_Is_In_Cache()
        {
            ...
            _memoryCache.TryGetValue(Arg.Is(deltaHash), out Arg.Any<Protocol.Delta.Delta>())
               .Returns(ci =>
                {
                    ci[1] = deltaFromCache;
                    return true;
            ...
            var found = _deltaCache.TryGetDelta(deltaHash, out var delta);
        ...
        public void TryGetDelta_Should_Hit_The_Dfs_When_Delta_Is_Not_In_Cache()
        {
            ...
            var found = _deltaCache.TryGetDelta(hash, out var delta);

            delta.Should().Be(deltaFromDfs);
            found.Should().BeTrue();

            _memoryCache.Received(1).TryGetValue(hash, out Arg.Any<Protocol.Delta.Delta>());
            _dfsReader.Received(1).TryReadDeltaFromDfs(hash, out Arg.Any<Protocol.Delta.Delta>());
        }
        ...
        {
            _memoryCache.TryGetValue(Arg.Is(hash), out Arg.Any<Protocol.Delta.Delta>())
               .Returns(false);
            _dfsReader.TryReadDeltaFromDfs(Arg.Is(hash), out Arg.Any<Protocol.Delta.Delta>())
               .Returns(ci =>
                {
                    ci[1] = deltaFromDfs;
                    return true;
