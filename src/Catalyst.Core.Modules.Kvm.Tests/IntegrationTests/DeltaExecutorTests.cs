...
namespace Catalyst.Core.Modules.Kvm.Tests.IntegrationTests
{
    public sealed class DeltaExecutorTests
    {
        ...
        public void Fails_when_sender_not_specified()
        {
            ...
            delta.PublicEntries[0].Signature = delta.PublicEntries[0].GenerateSignature(_cryptoContext, _senderPrivateKey, _signingContext);
        ...
        {
            var delta = EntryUtils.PrepareSingleContractEntryDelta(null, _senderPublicKey, 0, "0x60016000526001601FF300");
        ...
        {
            var delta = EntryUtils.PrepareSingleContractEntryDelta(null, _senderPublicKey, 0, "0x730001020304050607080910111213141516171819ff");
            delta.PublicEntries[0].GasLimit = 1_000_000L;
            delta.PublicEntries[0].Base.ReceiverPublicKey = ByteString.Empty;
            delta.PublicEntries[0].Signature = delta.PublicEntries[0].GenerateSignature(_cryptoContext, _senderPrivateKey, _signingContext);
