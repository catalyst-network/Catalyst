...
namespace Catalyst.Core.Modules.Rpc.Server.Tests.IntegrationTests.IO.Observers
{
    [TestFixture]
    ...
    public sealed class GetFileFromDfsObserverHandlerTests : FileSystemBasedTest
    {
        ...
        public async Task Get_File_Rpc(long byteSize)
        {
            ...
            try
            {
                var nodeAddress = MultiAddressHelper.GetAddress("sender");
                var rpcAddress = MultiAddressHelper.GetAddress("recipient");
                var peerSettings = Substitute.For<IPeerSettings>();
                peerSettings.Address.Returns(rpcAddress);
                var nodePeer = nodeAddress;
                var rpcPeer = rpcAddress;
