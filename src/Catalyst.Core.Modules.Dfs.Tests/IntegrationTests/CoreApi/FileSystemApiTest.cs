...
namespace Catalyst.Core.Modules.Dfs.Tests.IntegrationTests.CoreApi
{
    public class FileSystemApiTest
    {
        ...
        public async Task ReadWithOffset()
        {
            ...
            {
                using (var data = await ipfs.UnixFsApi.ReadFileAsync(node.Id, offset))
        ...
        public async Task Read_RawWithLength()
        {
            ...
            for (var offset = 0; offset < text.Length; ++offset)
            {
                ...
                {
                    using (var data = await ipfs.UnixFsApi.ReadFileAsync(node.Id, offset, length))
                    using (var reader = new StreamReader(data))
                    {
                        var readData = reader.ReadToEnd();
        ...
        public async Task Read_ChunkedWithLength()
        {
            ...
            for (var length = text.Length + 1; 0 < length; --length)
            {
                using (var data = await ipfs.UnixFsApi.ReadFileAsync(node.Id, 0, length))
                using (var reader = new StreamReader(data))
                {
                    var readData = reader.ReadToEnd();
        ...
        public async Task AddFile_WithProgress()
        {
            ...
            try
            {
                ...
                var result = await ipfs.UnixFsApi.AddFileAsync(path, options);
        ...
        public void AddDirectoryRecursive()
        {
            ...
            try
            {
                var dir = ipfs.UnixFsApi.AddDirectoryAsync(temp, true).Result;
                Assert.True(dir.IsDirectory);
