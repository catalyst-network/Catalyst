...
namespace Catalyst.Node.Modules.Core.P2P.Peer
{
    ...
    public class PeerManager : IDisposable
    {
        ...
        private async Task<bool> DataReceiver(Connection connection, CancellationToken cancelToken)
        {
            ...
            {
                LogException.Message("*** Data receiver cancelled " + connection.EndPoint.Address + ":" + connection.EndPoint.Port + " disconnected", e);
                throw;
            }
            catch (Exception e)
            {
                LogException.Message("*** Data receiver exception " + connection.EndPoint.Address + ":" + connection.EndPoint.Port + " disconnected", e);
                throw;
            }
            finally
            {                
...
//                            Log.Message("*** AcceptConnections SocketException " + ListenerIpAddress + " closed the connection.");
                            break;
                        default:
//                            Log.Message("*** AcceptConnections SocketException from " + peerIp.Ip + Environment.NewLine + ex);
                            break;
                    }
                }
                catch (Exception ex)
        ...
        private Connection StartPeerConnection(TcpClient tcpClient)
        {
            ...
            Log.Message("*** Connection created for " + connection.EndPoint.Address + connection.EndPoint.Port + " (now " + activeCount + " connections)");
            return connection;
        }
        ...
        private bool DisconnectConnection(Connection connection)
        {
            ...
            finally
            {
                ...
                Log.Message("***** Connection successfully disconnected connected (now " + activeCount + " connections active)");
            }
        }
