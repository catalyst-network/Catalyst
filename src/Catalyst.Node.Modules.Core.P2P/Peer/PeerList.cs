     ...
     public class PeerList : IEnumerable<Peer>
     {
        ...
        internal bool AddUnidentifiedConnectionToList(Connection needle)
        {
            ...
            try
            {
                if (UnIdentifiedPeers.TryGetValue(needle.EndPoint.Address + ":" + needle.EndPoint.Port, out Connection connection))
                {
                    ...
                    {
                        Log.Message("*** Active connection already exists for " + connection.EndPoint.Address + connection.EndPoint.Port);
                        return false;
                    }
                    try
                    {
                        ...
                        Log.Message("Removed stale connection for  " + connection.EndPoint.Address + connection.EndPoint.Port);
                    }
                    catch (ArgumentNullException e)
                    {
                        LogException.Message("AddUnidentifiedConnectionToList: RemoveUnidentifiedConnectionFromList", e);
                        needle.Dispose();
                        return false;
                    }
                }
            ...
            try
            {
                Log.Message("*** Unidentified connection " + needle.EndPoint.Address + needle.EndPoint.Port + " added to unidentified peer list)");
                AsyncRaiseEvent(OnAddedUnIdentifiedConnection, this, new NewUnIdentifiedConnectionEventArgs(needle));
            }
            catch (ArgumentNullException e)
            {
        ...
        internal bool RemoveUnidentifiedConnectionFromList(Connection connection)
        {
            ...
            try
            {
                ...
                {
                    Log.Message("***** Successfully removed " + removedConnection.EndPoint.Address + removedConnection.EndPoint.Port);
                    return true;
                }
                Log.Message("*** unable to find connection " + connection.EndPoint.Address+":"+connection.EndPoint.Port);
                return false;
            }
            catch (ArgumentNullException e)
            {
