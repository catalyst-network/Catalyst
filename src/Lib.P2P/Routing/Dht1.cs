...
namespace Lib.P2P.Routing
{
    ...
    {
        private static ILog log = LogManager.GetLogger(typeof(Dht1));
        ...
        public Task StartAsync()
        {
            ...
            foreach (var peer in Swarm.KnownPeers) RoutingTable.Add(peer);
        ...
        /// </summary>
        private void Swarm_PeerDiscovered(object sender, Peer e) { RoutingTable.Add(e); }

        /// <summary>
        ///   The swarm has removed a peer, update the routing table.
        /// </summary>
        private void Swarm_PeerRemoved(object sender, Peer e) { RoutingTable.Remove(e); }
        ...
        public async Task<Peer> FindPeerAsync(MultiHash id, CancellationToken cancel = default(CancellationToken))
        {
            ...
            if (dquery.Answers.Count() == 0) return RoutingTable.NearestPeers(id).FirstOrDefault();
            ...
            CancellationToken cancel = default(CancellationToken))
        {
            ...
            if (action != null) dquery.AnswerObtained += (s, e) => action.Invoke(e);
            ...
            foreach (var provider in providers) dquery.AddAnswer(provider);

            // Ask our peers for more providers.
            if (limit > dquery.Answers.Count()) await dquery.RunAsync(cancel).ConfigureAwait(false);
        ...
        {
            if (request.ProviderPeers == null) return null;
            Cid cid;
            try
            {
