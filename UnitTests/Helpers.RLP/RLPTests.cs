...
namespace ADL.RLP.UnitTests
{
    public class RLPTests
    {
        private static void AssertStringEncoding(string test, string expected)
        {
            ...
            var encoderesult = ADL.RLP.RLP.EncodeElement(testBytes);
            Assert.Equal(expected, encoderesult.ToHex());

            var decodeResult = ADL.RLP.RLP.Decode(encoderesult)[0].RLPData;
        ...
        {
            var encoderesult = ADL.RLP.RLP.EncodeList(EncodeElementsBytes(test.ToBytesForRLPEncoding()));
            Assert.Equal(expected, encoderesult.ToHex());

            var decodeResult = ADL.RLP.RLP.Decode(encoderesult)[0] as RLPCollection;
        ...
        private static byte[][] EncodeElementsBytes(params byte[][] bytes)
        {
            ...
            foreach (var byteElement in bytes)
                encodeElements.Add(ADL.RLP.RLP.EncodeElement(byteElement));
        ...
        public void ShouldEncodeEmptyList()
        {
            var test = new byte[0][];
            var expected = "c0";
            var encoderesult = ADL.RLP.RLP.EncodeList(test);
            Assert.Equal(expected, encoderesult.ToHex());
