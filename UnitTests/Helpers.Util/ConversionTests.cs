...
namespace ADL.Util.UnitTests
{
    public class ConversionTests
    {
        ...
        public void ShouldConvertFromDecimalUnit()
        {
            var unitConversion = new UnitConversion();
            var ether = 0.0010m;
            var wei = UnitConversion.Convert.ToWei(ether, UnitConversion.AtlasUnit.Atlaser);
        ...
        [Fact]
        public void ShouldConvertFromWeiAndBackToAtlas()
        {
            var unitConversion = new UnitConversion();
            var val = BigInteger.Parse("1000000000000000000000000001");
            var result = unitConversion.FromWei(val, 18);
        ...
        public void ShouldConvertPeriodicGether()
        {
            var unitConversion = new UnitConversion();
            var ether = (decimal) 1 / 3;
            var wei = UnitConversion.Convert.ToWei(ether, UnitConversion.AtlasUnit.Gether);
            var val = BigInteger.Parse("3".PadLeft(27, '3'));
            var result = unitConversion.FromWei(val, UnitConversion.AtlasUnit.Gether);
            Assert.Equal(UnitConversion.Convert.ToWei(result, UnitConversion.AtlasUnit.Gether), wei);
        }
        ...
        public void ShouldConvertPeriodicTether()
        {
            ...
            var wei = UnitConversion.Convert.ToWei(ether, UnitConversion.AtlasUnit.Tether);
            var val = BigInteger.Parse("3".PadLeft(30, '3'));
            var result = unitConversion.FromWeiToBigDecimal(val, UnitConversion.AtlasUnit.Tether);
            Assert.Equal(UnitConversion.Convert.ToWei(result, UnitConversion.AtlasUnit.Tether), wei);
        }
        ...
        [Fact]
        public void ShouldConvertToWeiUsingNumberOfDecimalsAtlas()
        {
            var unitConversion = new UnitConversion();
            var val = BigInteger.Parse("1000000000000000000000000001");
            var result = unitConversion.FromWei(val, 18);
